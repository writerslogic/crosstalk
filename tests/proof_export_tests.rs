...
fn to_lean4_includes_invariant_name() {
    ...
    assert!(
        out.contains("turns_ordered"),
        "output must contain the invariant name"
    );
}
...
async fn export_all_proofs_creates_lean_file() {
    ...
    ProofExporter::export_all_proofs(dir.path().to_str().unwrap())
        .await
        .unwrap();
...
async fn exported_file_contains_all_three_invariants() {
    ...
    assert!(
        content.contains("monotonic_indices"),
        "must export monotonic_indices"
    );
    assert!(
        content.contains("hash_chain_integrity"),
        "must export hash_chain_integrity"
    );
    assert!(
        content.contains("artifact_version_consistency"),
        "must export artifact_version_consistency"
    );
}
...
async fn exported_file_has_namespace_and_imports() {
    ...
    assert!(
        content.contains("namespace Crosstalk"),
        "must open Crosstalk namespace"
    );
    assert!(
        content.contains("end Crosstalk"),
        "must close Crosstalk namespace"
    );
}
...
async fn lean_type_check_when_available() {
    ...
    assert!(
        status.success(),
        "Invariants.lean must type-check without errors"
    );
}
