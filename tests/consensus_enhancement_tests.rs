use crosstalk::engines::consensus::{NashSolver, KalmanConvergence, CertaintyAnalyzer};
...
fn test_kalman_adaptive_noise_rejection() {
    ...
    assert!(delta1 > delta2, "High certainty update should move the state more than low certainty update");
}

#[test]
fn test_certainty_analyzer_sovereign_heuristic() {
    let confident = "I have successfully implemented the proof and verified it matches all invariants.";
