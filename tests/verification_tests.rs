...
fn test_invariant_checker_monotonic_indices() {
    ...
    state.turns.push(Turn { index: 1, model_id: "m".to_string(), content: "c".to_string(), timestamp: 0, diffs: vec![], certainty: None, outcome: TurnOutcome::Unknown, task_category: None, structure: None, signature: vec![], surprise_signal: None });
    state.turns.push(Turn { index: 0, model_id: "m".to_string(), content: "c".to_string(), timestamp: 0, diffs: vec![], certainty: None, outcome: TurnOutcome::Unknown, task_category: None, structure: None, signature: vec![], surprise_signal: None });
...
fn test_tautology_filter_detects_similar() {
    ...
    assert!(TautologyFilter::is_tautological("This is a long sentence that repeats itself very often.", &history));
}

#[test]
fn test_hash_chain_determinism() {
    let mut s1 = ConversationState::new("test");
    s1.artifacts.insert("a".to_string(), Arc::new(Artifact {
        name: "a".to_string(),
        content: "c".to_string(),
