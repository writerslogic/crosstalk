...
where
    ...
    <M as rig::completion::CompletionModel>::StreamingResponse: rig::wasm_compat::WasmCompatSend + rig::completion::GetTokenUsage,
    ...
    T: rig::completion::Prompt + rig::streaming::StreamingPrompt<M, R, Hook = ()> + Send + Sync,
{
    ...
    fn stream_prompt<'a>(
        ...
        Box::pin(async move {
            ...
            let mapped_stream = stream.filter_map(|res| {
                match res {
                    Ok(rig::agent::MultiTurnStreamItem::StreamAssistantItem(
                        rig::streaming::StreamedAssistantContent::Text(text)
                    )) => futures::future::ready(Some(Ok(text.text))),
                    Ok(_) => futures::future::ready(None),
                    Err(e) => futures::future::ready(Some(Err(anyhow::anyhow!("Stream error: {:?}", e)))),
                }
            });

            Ok(Box::pin(mapped_stream) as Pin<Box<dyn Stream<Item = Result<String, anyhow::Error>> + Send + 'a>>)
        })
    }
}
