use crate::agent_trait::{AgentWrapper, PromptAgent};
    ...
    pub fn create_agent(model_id: &str) -> Result<Box<dyn PromptAgent>> {
        if model_id.starts_with("gemini") {
            ...
            Ok(Box::new(AgentWrapper::<
                ...
                rig::providers::gemini::streaming::StreamingCompletionResponse,
                _,
    ...
    pub fn check_env(models: &[String]) -> Result<()> {
        for m in models {
            if m.starts_with("gemini") && env::var("GEMINI_API_KEY").is_err() {
                return Err(anyhow!(
                    "Environment Variable GEMINI_API_KEY is not set for {}",
                    m
                ));
            }
            if m.starts_with("gpt") && env::var("OPENAI_API_KEY").is_err() {
                return Err(anyhow!(
                    "Environment Variable OPENAI_API_KEY is not set for {}",
                    m
                ));
            }
        }
