...
impl ModelFactory {
    pub fn create_agent(model_id: &str) -> Result<Box<dyn PromptAgent>> {
        let base_id = if let Some(i) = model_id.find('#') {
            &model_id[..i]
        } else {
            model_id
        };
        ...
        if model_id_lower.contains("sonnet")
            || model_id_lower.contains("claude")
            || model_id_lower.contains("opus")
            || model_id_lower.contains("haiku")
        {
        ...
        } else if model_id_lower.starts_with("gpt")
            || model_id_lower.starts_with("o1")
            || model_id_lower.starts_with("o3")
            || model_id_lower.starts_with("o4")
            || model_id_lower.starts_with("chat")
        {
            ...
            Ok(Box::new(AgentWrapper::<
                ...
                _,
            >::new(model_id.to_string(), agent)))
        } else if model_id_lower.starts_with("codestral")
            || model_id_lower.starts_with("mistral")
            || model_id_lower.starts_with("pixtral")
            || model_id_lower.starts_with("ministral")
        {
            ...
            Ok(Box::new(AgentWrapper::<
                ...
                _,
            >::new(model_id.to_string(), agent)))
        } else if model_id_lower.starts_with("llama")
            || model_id_lower.starts_with("gemma")
            || model_id_lower.starts_with("mixtral")
        {
    ...
    fn create_openrouter_agent(
        ...
        let api_key = require_env("OPENROUTER_API_KEY")
            .or_else(|_| {
                tracing::warn!(
                    model = model_id,
                    "OPENROUTER_API_KEY not set, falling back to LLM_API_KEY"
                );
                require_env("LLM_API_KEY")
            })
            .map_err(|_| {
                anyhow::anyhow!(
                    "Missing OPENROUTER_API_KEY (and LLM_API_KEY fallback) for model {}",
                    model_id
                )
            })?;
    ...
    pub fn openrouter_path(model_id: &str) -> Option<String> {
        ...
        } else if lower.starts_with("codestral")
    ...
    pub fn check_env(model_ids: &[String]) -> Result<()> {
        for model_id in model_ids {
                ...
                || model_lower.contains("haiku")
            {
                ...
                || model_lower.starts_with("chat")
            {
                "OPENAI_API_KEY"
            } else if model_lower.starts_with("deepseek") {
                "DEEPSEEK_API_KEY"
            } else if model_lower.starts_with("codestral")
            ...
            std::env::var(key)
                ...
                .map_err(|_| {
                    anyhow::anyhow!("Missing {key} for model {model_id}. Get one at {url}")
                })?;
        }
        Ok(())
    }
